 ...
 */
void Servo::Enable() { HAL_TIM_PWM_Start(servo_def[servo_number].htim, servo_def[servo_number].channel); }

/*!
 * @brief Выключение сервопривода
 */
void Servo::Disable() { HAL_TIM_PWM_Stop(servo_def[servo_number].htim, servo_def[servo_number].channel); }
